...
| **20+ Languages** | TypeScript, JavaScript, ArkTS, Python, Go, Rust, Java, C#, VB.NET, PHP, Ruby, C, C++, CUDA, Objective-C, Metal, Swift, Kotlin, Scala, Dart, Lua, Luau, R, Nix, Erlang, CFML, COBOL, Solidity, Terraform/OpenTofu, Starlark (Bazel), Svelte, Vue, Astro, Liquid, Pascal/Delphi |
...
| Starlark (Bazel) | `.bzl`, `.star`, `BUILD`, `BUILD.bazel`, `WORKSPACE`, `WORKSPACE.bazel`, `MODULE.bazel` | Full support (build targets, macro/rule `def`s, `load()` imports; `srcs`/`hdrs`/`data` — incl. simple `glob([...])` — linked to the real source files they build, and `deps` linked to the other targets they depend on, so a target connects to the actual function/class graph of the language it builds) |
